...
! function(d) {
    ...
    a.DEFAULTS = {
        ...
        var t = this;
        this.$btnToolbar = d('[data-responsive-table-toolbar="' + this.id + '"]').addClass("btn-toolbar"), 0 === this.$btnToolbar.length && (this.$btnToolbar = d('<div class="btn-toolbar" />')), this.$dropdownGroup = d('<div class="btn-group dropdown-btn-group pull-right" />'), this.$dropdownBtn = d('<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">' + this.options.i18n.display + ' <span class="caret"></span></button>'), this.$dropdownContainer = d('<ul class="dropdown-menu"/>'), this.options.addFocusBtn && (this.$focusGroup = d('<div class="btn-group focus-btn-group" />'), this.$focusBtn = d('<button type="button" class="btn btn-default">' + this.options.i18n.focus + "</button>"), this.options.focusBtnIcon && this.$focusBtn.prepend('<span class="' + this.options.focusBtnIcon + '"></span> '), this.$focusGroup.append(this.$focusBtn), this.$btnToolbar.append(this.$focusGroup), this.$focusBtn.on('click',function() {
            d.proxy(t.activateFocus(), t)
        }), this.$bodyRows.on('click',function() {
            d.proxy(t.focusOnRow(d(this)), t)
        })), this.options.addDisplayAllBtn && (this.$displayAllBtn = d('<button type="button" class="btn btn-default">' + this.options.i18n.displayAll + "</button>"), this.$dropdownGroup.append(this.$displayAllBtn), this.$table.hasClass("display-all") && this.$displayAllBtn.addClass("btn-primary"), this.$displayAllBtn.on('click',function() {
        ...
        n.$hdrCells.each(function(t) {
            ...
            if (e || (e = n.idPrefix + t, i.prop("id", e)), "" === o && (o = i.attr("data-col-name")), i.is("[data-priority]") && -1 !== i.data("priority")) {
                var a = d('<li class="checkbox-row"><input type="checkbox" name="toggle-' + e + '" id="toggle-' + e + '" value="' + e + '" /> <label for="toggle-' + e + '">' + o + "</label></li>"),
                    s = a.find("input");
                n.$dropdownContainer.append(a), a.on('click',function() {
                    s.prop("checked", !s.prop("checked")), s.trigger("change")
                }), d("html").hasClass("lt-ie9") && s.on('click',function() {
                    d(this).trigger("change")
                }), a.find("label").on('click',function(t) {
                    t.stopPropagation()
                }), a.find("input").on('click',function(t) {
                    t.stopPropagation()
                }).change(function() {
                    var e = d(this),
                        t = e.val(),
